...
class Evaluator:
    ...
    def retrain(self, pre_block):
        ...
        # softmax
        logits = self._makedense(logits, ('', [self.NUM_CLASSES], 'relu'))
...
if __name__ == '__main__':
    ...
    graph_full = [[1], [2], [3]]
    cell_list = [Cell('conv', 32, 3, ''), Cell('conv', 32, 1, ''), Cell('conv', 48, 5, '')]
    eval.set_data_size(10000)
    network1 = NetworkItem(0, graph_full, cell_list, "")
    # e = eval.evaluate(network1, is_bestNN=True)
    # print(e)
    graph_full = [[1], [2], [3]]
    cell_list = [Cell('conv', 128, 3, ''), Cell('conv', 64, 1, ''), Cell('conv', 48, 5, '')]

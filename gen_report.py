...
def gen_report(time_for_run, date, place, item_path, relative_item_path, setting):
	...
	with open(os.path.join(item_path, 'nas_log.txt'), "r") as f:
		...
		# find the final network structure
	...
	content += "#### 过程统计\n\n"
	content += "每轮信息：\n\n"
	content += "| 进度 | 数据量     | 最高评分    |   网络个数    |   总用时/s     |  每个网络平均用时/s|\n"
	content += "|----| --------    | -----:      |   -----     |   ----         |     -----       |\n"
...
if __name__ == '__main__':
	report_path = "./exp_report/exp_repo_restruct/report01_06.md"
		...
		for item in sub_items:
			if ('OK' in item or 'ok' in item) and '-1' not in item:
				...
				with open(os.path.join(item_path, 'nas_config.json')) as f:
					setting = json.load(f)
				report_content = gen_report(time_for_run, date, place, item_path, relative_item_path, setting)
